...
describe('Users E2E Tests:', function () {
  ...
  describe('Signup Validation', function () {
    ...
    it('Should report a password with less than 10 characters long - "P@$$w0rd!"', function () {
      ...
      element(by.css('button[type=submit]')).click();
      // Password Error
      expect(element.all(by.css('.error-text')).get(0).getText()).toBe('Le mot de passe doit contenir au moins une majuscule.');
    });

    it('Should report a password with greater than 128 caractères.', function () {
      browser.get('http://localhost:3001/authentication/signup');
      // Enter First Name
      element(by.model('credentials.firstName')).sendKeys(user1.firstName);
      // Enter Last Name
    ...
    it('Should report a password with more than 3 or more repeating characters - "P@$$w0rd!!!"', function () {
      ...
      element(by.css('button[type=submit]')).click();
      // Password Error
      expect(element.all(by.css('.error-text')).get(0).getText()).toBe('Le mot de passe ne doit pas contenir plus de 2 caractères identiques consécutifs.');
    });
    ...
    it('Should report a password with less than one number - "P@$$word!!"', function () {
      ...
      element(by.css('button[type=submit]')).click();
      // Password Error
      expect(element.all(by.css('.error-text')).get(0).getText()).toBe('Le mot de passe doit contenir au moins un nombre.');
    });
    ...
    it('Should report a password with less than one special character - "Passw0rdss"', function () {
      ...
      element(by.css('button[type=submit]')).click();
      // Password Error
      expect(element.all(by.css('.error-text')).get(0).getText()).toBe('Le mot de passe doit contenir au moins un caractère spécial.');
    });
